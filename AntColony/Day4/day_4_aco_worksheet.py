...
filepath = f"{basepath}/Database/{args.dataset}"
...
def fitness(D, solution): # Defines fitness as the sum of all routes
    ...
    for route in solution:
        ...
        for i in range(len(route) - 1):
            ...
            to_node = route[i + 1]
            route_distance += D[from_node][to_node]
...
def compute_tau0(distance_matrix): # KNN-based tau0 from sym15112020
    ...
    for i in range(num_nodes):
        ...
        for j in range(num_nodes):
            if i != j:
                distances_from_i.append(distance_matrix[i][j])

        nearest_distance = min(distances_from_i)
        nearest_distances.append(nearest_distance)

    total_nearest = sum(nearest_distances)
    return 1.0 / total_nearest

def construct_solution(tau, eta, demand, capacity, candidate_list, alpha, beta, rho, tau0, h):
    n = len(tau)
    depot = 0
    ...
    while unvisited:
        ...
        while True:
            ...
            local_update(tau, current, next_node, rho, tau0) 
...
def two_opt(route, D): # two-opt local search mechanism implementation
    best_route = route
    best_cost = route_cost(route, D)
    improved = True
    ...
    while improved:
        ...
        for i in range(1, n - 2):          # skip depot at index 0
            for j in range(i + 1, n - 1):  # avoid breaking end depot

                if j - i == 1: # Skip adjacent edges (no real change)
                    continue

                # Create new route via 2-opt swap
                new_route = (
                    best_route[:i] +
                    best_route[i:j][::-1] +
                    best_route[j:]
...
def local_update(tau, current, next_node, rho, tau0): # Local pheromone update as per equation
    tau[current][next_node] = (1 - rho) * tau[current][next_node] + rho * tau0
...
def local_search(solution, D):
    new_solution = []
    for r in solution:
        new_route = two_opt(r, D)
...
def ant_solver(vertices, demand, capacity, alpha, beta, rho, h, Q, ants):
    ...
    tau0 = compute_tau0(D)
    tau = np.full((len(vertices), len(vertices)), tau0)
    ...
    for it in range(args.generations):
        ...
        for _ in range(ants):
            solution = construct_solution(tau, eta, demand, capacity, candidate_list, alpha, beta, rho, tau0, h)
            
            solution = local_search(solution, D)
        ...
        global_update(tau, solutions, fitnesses, rho, Q)
        history.append(best_cost)
        solutions_history.append(copy.deepcopy(best_solution))
    
    return best_solution, best_cost, history, solutions_history
...
def plot_results(vertices, solution, distance_over_time, distance_found, filename, shortest, algo, exval):
    ...
    fig.suptitle(f"Vehicle Routing Solution using Ant Colony Algorithm for: {name}", fontsize=12)
...
def animate_results(vertices, solutions_over_time, distance_over_time, distance_found, filename, shortest, algo, exval):
    ...
    def update(frame):
        ...
        axs[1].text(
            0.5,
            -0.25,
            f"Shortest Distance possible: {shortest}",
        ...
        axs[1].text(
            0.5,
            -0.30,
            f"Best Distance Found: {distance_found:.0f}",
            transform=axs[1].transAxes,
            ha='center'
        )
    ...
    ani = FuncAnimation(
        fig,
        update,
        frames=len(solutions_over_time),
...
if __name__ == "__main__":
    ...
    best_solution, best_cost, history, solutions_history = ant_solver(vertices,demand,capacity,args.alpha,args.beta,args.rho,args.h,args.Q,args.ants)    
    ...
    animate_results(vertices,solutions_history,history,best_cost,filepath,shortest,algo="aco",exval=args.exval)