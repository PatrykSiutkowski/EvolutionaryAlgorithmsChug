...
class TestEnvironment(unittest.TestCase):
    ...
    def test_mp4_recognized_and_playable(self): # Verify that .mp4 files are recognized and can be opened by ffplay.
        # Check MIME type recognition
        mime_type, _ = mimetypes.guess_type("video.mp4")
        self.assertEqual(mime_type, "video/mp4", ".mp4 extension is not recognized as video/mp4")

        # Create an empty placeholder mp4 file
        # (Recognition test only; playback requires a real MP4)
        with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as tmp:
            mp4_file = tmp.name

        try:
            # Check if ffplay is installed
            subprocess.run(["ffplay", "-version"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, check=True, shell=os.name == "nt")

            # Attempt to open the file
            result = subprocess.run(["ffplay", "-autoexit", "-nodisp", "-loglevel", "error", mp4_file], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, shell=os.name == "nt")

            # ffplay will fail because the file is empty.
            # What we're testing is that it recognizes the .mp4 extension
            # and attempts to open it.
            self.assertNotEqual(result.returncode, 127, "ffplay could not be executed.")

        except FileNotFoundError:
            self.skipTest("ffplay is not installed.")
